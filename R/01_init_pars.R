init_pars <- function(X, Y, G = NULL, sigma_0 = 1, sigma_slab = 2, damping = 0.9,
                      standardize, intercept, timeseries, maxdelay) { # , sigmoid_function="exact"
  ...
  # V2_slash <- V2
  # V2_slash_new <- 1/(1/diag(V) - 1/V2) #### schon im ersten Schritt negativ???
  # indices <- which(V2_slash_new > 0)
  # V2_slash[indices] <- V2_slash_new[indices]
  
  pars <- list(V1_inv = V1_inv, V1_inv_m1 = V1_inv_m1, p0 = p0, rho4 = rho4, V2 = V2, V2_inv = V2_inv, V2_inv_m2 = V2_inv_m2, r2 = r2, r3 = r3, rho3 = rho3, V = V, m = m, r = r, rho = rho, predicterror=predicterror, sigma2_slab = sigma2_slab, sigma2_0 = sigma2_0, X = X, Y = Y, M = M, N = N, G = G, groups = groups, damping = damping, intercept=intercept, timeseries=timeseries, maxdelay=maxdelay, iterations=0) # V2 is needed for Woodbury formula # , sigmoid_function=sigmoid_function
