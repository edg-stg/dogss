iter_pars <- function(pars) {
  ...
  if (M!=length(groups)) { # if length(groups)==M, we don't have any groups, so we don't update anything for f3tilde and ignore f4tilde
    # update for f3tilde first... find pars of marginal q3slash distribution
    ...
    rho_new <- pars$rho4 + sapply(groups, function(g) sum(pars$rho3[G==g]))
    pars$r <- r_new
    pars$rho <- rho_new
  }
  ...
  ## if there are terms with negative variance V2_slash, we do not perform the update for
  ## these terms (resp. doing it with the old V2_slash?):
  ...
  # derive pars of the q-new-distribution: not needed (but p_aux)! derive updated pars of f2tilde directly:
  
  r2_new <- 0.5*(log(V2_slash) - log(V2_slash+sigma2_slab) + m2_slash^2*(1/V2_slash - 1/(V2_slash + sigma2_slab)))
  p_aux <- sigmoid(r2_new+r2_slash) # , pars$sigmoid_function
  
  a <- p_aux*m2_slash/(V2_slash+sigma2_slab) + (1-p_aux)*m2_slash/V2_slash
  b <- p_aux*(m2_slash^2-V2_slash-sigma2_slab)/(V2_slash+sigma2_slab)^2 + (1-p_aux)*(m2_slash^2-V2_slash)/V2_slash^2
  ...
  if (M > N) { # more features than observations: Woodbury trick
    # Lambda <- diag(as.vector(pars$V2), nrow=M)
    # Sigma2D <- diag(sigma2_0, N)
    ...
    # pars$V[lower.tri(pars$V)] <- t(pars$V)[lower.tri(pars$V)]
    # rm(Lambda, Sigma2D)
    # XLambda <- t(t(X)*as.vector(pars$V2)) # that's the same like X %*% Lambda, but much more efficient
    # sigma2pXLambdaXt <- diag(sigma2_0, N) + CppEigenMultiplication(XLambda, t(X)) # XLambda %*% t(X) # tcrossprod(XLambda, X) #
    # Inv <- CppEigenInv(sigma2pXLambdaXt) # solve(sigma2pXLambdaXt, tol = 1e-100)
    # rm(sigma2pXLambdaXt)
    # LInv <- Inv
    # LInv[upper.tri(LInv)] <- 0
    # diag(LInv) <- diag(LInv)/2
    # # t(XLambda) %*% LInv %*% XLambda
    # pars$V <- Lambda - CppEigenMultiplicationThree(XLambda, LInv) # CppMultiplicationThree(t(XLambda), Inv, XLambda) # tcrossprod(crossprod(XLambda, Inv), t(XLambda)) #
    # rm(Lambda, XLambda, Inv)
    #   C <- tryCatch({chol(sigma2pXLambdaXt)}, error = function(e) {return(NULL)})
    #   Inv <- 'if'(is.null(C), solve(sigma2pXLambdaXt), chol2inv(C)) #for solve: , tol = 1e-100
    #   if (is.null(C)) cat("cholesky didn't work!\n")
    #   V <- Lambda - t(XLambda) %*% Inv %*% XLambda
  } else { # more observations N than features M: calculate V directly
    ...
    #   C <- tryCatch({chol(V1_inv+diag(1/V2))}, error = function(e) {return(NULL)})
    #   Inv <- 'if'(is.null(C), solve(V1_inv+diag(1/V2)), chol2inv(C)) #for solve: , tol = 1e-100
    #   if (is.null(C)) cat("cholesky didn't work!\n")
    #   V <- Inv
  }
  ...
  if (any(pars$r>0)) {
    pars$predicterror <- sum((Y-X[, pars$r>0, drop=FALSE]%*%pars$m[pars$r>0])^2)
  } else {
